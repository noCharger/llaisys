...
def benchmark(torch_func, llaisys_func, device_name, warmup=10, repeat=100):
    ...
    torch_latencies = time_op(torch_func)
    llaisys_latencies = time_op(llaisys_func)
    ...
    def print_metrics(name, latencies):
        ...
        print(f"        {name} time: {mean:.5f} ms (P50: {p50:.5f}, P90: {p90:.5f}, P99: {p99:.5f}, Std: {std:.5f})")

    print_metrics("Torch", torch_latencies)
