...
class TestClipperScheduler:
    ...
    async def test_delayed_batching(self):
        mock_model = MagicMock()
        mock_model.forward_batch = AsyncMock(return_value=[1, 2])
        ...
        job1 = RequestJob(request_id="1", session_ptr="s1", input_ids=[1], temperature=0.7, top_p=0.9, top_k=50)
        await scheduler.submit(job1)

        assert not job1.future.done()

        job2 = RequestJob(request_id="2", session_ptr="s2", input_ids=[2], temperature=0.7, top_p=0.9, top_k=50)
